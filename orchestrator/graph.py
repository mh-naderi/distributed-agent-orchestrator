...
# Nodes and routing
# ---------------------------------------------------------------------------
# These live at module level because none of them touch the registry or the
# provider - they are pure functions of the conversation state. Only call_llm
# and call_mcp_tool need what build_graph closes over, so only those two stay
# nested. Keeping the rest here means a routing decision can be tested by
# calling it, instead of scripting a whole provider and running the graph to
# observe one branch.
...
def build_graph(registry: MCPToolRegistry, provider: LLMProvider):
    ...
    module level, so a branch can be checked by calling it rather than by
    scripting a provider and reading the transcript.
    """
