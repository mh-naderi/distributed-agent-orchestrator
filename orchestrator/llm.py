...
This module is its own file, behind a Protocol, so a second provider can be
added without the graph changing at all. One was: an Anthropic provider, for
the runs where a 1.7B local model is not good enough. It was removed - see
"Decision: the escalation path was removed rather than left unverified" in
docs/architecture.md - and the seam it proved is what stayed.
"""
...
# ---------------------------------------------------------------------------
# The graph only ever sees these, never a provider's own types. That is what
...
class LLMResponse:
    ...
    # A provider that must replay its own output verbatim would need the
    # original blocks carried here. None does, now: the Claude provider that
    # needed it was removed, along with the provider-specific key it put into
    # every message the graph produced. The field went with it rather than
    # staying as scaffolding for a provider that does not exist.
...
class OllamaProvider:
    ...
    def _to_ollama_messages(messages: list[dict]) -> list[dict]:
        ...
        keeps a second provider a drop-in: the conversion is the only thing
        that has to be written, and nothing in state has to change.
        """
        converted = []
...
def get_provider() -> LLMProvider:
    ...
    One provider, and a Protocol above it. A hosted-model escalation path used
    to live here behind an `escalate` flag; it was removed because it had never
    made a real call and could not be verified without spending money this
    project has ruled out. This function is the seam where a second provider
    goes when there is a way to prove it works.
    """
